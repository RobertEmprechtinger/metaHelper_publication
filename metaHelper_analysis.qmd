...
```{r}
dat_result %>%
  mutate(result_correct = as.factor(result_correct)) %>%
...
```{r}
summary(result_all)$fixed["groupIntervention", ]
```
...
tidy_samples %>%
  mutate(metaHelper = logit2prob(b_Intercept + b_groupIntervention) *100,
...
tidy_samples %>%
  mutate(metaHelper = b_Intercept + b_groupIntervention,
         Control = b_Intercept) %>%
  ggplot(aes(x = b_groupIntervention)) +
  xlim(c(0, 4)) +
  geom_density(fill = "gray", alpha = 0.7, color = NA) +
  theme_light() +
  xlab("Posterior Distribution Effect Intervention (logit)") +
...
tidy_samples %>%
  ...
  geom_density(fill = "gray", alpha = 0.7, color = NA) +
  theme_light() +
  xlab("Posterior Distribution Effect Intervention (Seconds)") +
  geom_vline(xintercept = 0, color = "red")
```
