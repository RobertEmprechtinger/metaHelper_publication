...
dat_result  %>%
  ...
  scale_color_manual(values = c(color_correct, color_false)) + 
  theme_light()
...
dat_result %>%
  ...
  geom_rect(xmin = 0, xmax = 14.5, ymin = 0, ymax = 5000, fill = color_metahelper,  alpha = 0.02) +
  geom_rect(xmin = 14.5, xmax = 500, ymin = 0, ymax = 5000, fill = "#F79824",  alpha = 0.02) +
  geom_point(aes(color = result_correct)) +
  facet_wrap(~task, scales = "free", ncol = 2) +
  xlab("Participant ID") +
  ylab("Absolute Result Value") +
  labs(color = "Result") +
  scale_fill_manual(name = "Group",
                    values = c("#F79824", color_metahelper)) +  
...
tidy_samples_sens_ID %>%
  mutate(
    # Calculate probability for the control group (the intercept)
    Control = logit2prob(b_Intercept) * 100,
    # Calculate probability for the metaHelper group (intercept + effect)
    metaHelper = logit2prob(b_Intercept + b_groupmetaHelper) * 100
  ) %>%
  # Keep only the columns we need for the plot
  select(Control, metaHelper) %>%
  # Reshape data into a long format for ggplot
  pivot_longer(everything(), names_to = "Group", values_to = "PercentageCorrect") %>%
  # Create the plot
  ggplot(aes(x = PercentageCorrect, fill = Group)) +
